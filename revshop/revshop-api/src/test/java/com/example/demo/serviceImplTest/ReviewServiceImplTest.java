...
class ReviewServiceImplTest {
	...
	void addReview_shouldAddSuccessfully() {
		...
		when(reviewRepository.findByProduct(product)).thenReturn(List.of()); // for updateProductRating
	...
	void addReview_shouldThrow_whenAlreadyReviewed() {
		...
		when(reviewRepository.findByUserAndProduct(user, product)).thenReturn(Optional.of(new Review()));

		assertThrows(RuntimeException.class, () -> reviewService.addReview(1L, request));
	}
	...
	void getReviewStats_shouldReturnCorrectStats() {
		...
		when(reviewRepository.countByProduct(product)).thenReturn(2L);

		when(reviewRepository.findByProduct(product)).thenReturn(List.of(r1, r2));
