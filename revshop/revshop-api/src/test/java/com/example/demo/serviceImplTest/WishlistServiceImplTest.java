...
class WishlistServiceImplTest {
	...
	void toggleWishlist_shouldRemoveProduct_whenExists() {
		...
		when(wishlistRepository.findByUserIdAndProductId(1L, 2L)).thenReturn(Optional.of(wishlist));
	...
	void getUserWishlistProductIds_shouldReturnProductIds() {
		...
		when(wishlistRepository.findByUserId(1L)).thenReturn(List.of(w1, w2));
	...
	void isWishlisted_shouldReturnTrue_whenExists() {
		...
		when(wishlistRepository.findByUserIdAndProductId(1L, 2L)).thenReturn(Optional.of(new Wishlist(1L, 2L)));
